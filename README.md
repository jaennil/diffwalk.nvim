...
- **Reading what was removed** — deleted lines are drawn as virtual lines, and the cursor cannot enter virtual text. `:DiffwalkOld` opens the file as it was at the base in a real buffer next to it, both sides in diff mode, so a removed signature can be walked, searched and yanked — with a blame of the base version on the line under the cursor, which is the only way to see who wrote a line that no longer exists.
...
require("diffwalk").setup({
  ...
  blame = true,      -- who wrote the line, in the base version of a file
